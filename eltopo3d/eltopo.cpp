                               ...
                               struct ElTopoMesh* outputs )
{
    ...
    {
        defrag_info->vertex_is_remove[i] = surface_tracker.m_vertex_change_history[i].is_remove ? 1 : 0;
        defrag_info->vertex_index[i] = to_int(surface_tracker.m_vertex_change_history[i].vertex_index);
        defrag_info->split_edge[2*i+0] = to_int(surface_tracker.m_vertex_change_history[i].split_edge[0]);
        defrag_info->split_edge[2*i+1] = to_int(surface_tracker.m_vertex_change_history[i].split_edge[1]);
    }
    ...
    {
        defrag_info->triangle_is_remove[i] = surface_tracker.m_triangle_change_history[i].is_remove ? 1 : 0;
        defrag_info->triangle_index[i] = to_int(surface_tracker.m_triangle_change_history[i].triangle_index);
        defrag_info->new_tri[3*i+0] = to_int(surface_tracker.m_triangle_change_history[i].tri[0]);
        defrag_info->new_tri[3*i+1] = to_int(surface_tracker.m_triangle_change_history[i].tri[1]);
        defrag_info->new_tri[3*i+2] = to_int(surface_tracker.m_triangle_change_history[i].tri[2]);
    }
    

    assert( "Mesh defrag info not present in C-API" );
                       ...
                       double *out_dt )
{
    ...
    DynamicSurface dynamic_surface(vs, 
                                   ts, 
                                   masses, 
                                   general_options->m_proximity_epsilon, 
                                   options->m_friction_coefficient, 
                                   general_options->m_collision_safety, 
                                   general_options->m_verbose );
